...
abstract contract RewardDistributionV2 is RewardDistributionV2Storage {
    ...
    modifier onlyDistributor(address token) {
        if (token == poolCreator.getMCBToken()) {
    ...
    function __RewardDistributionV2_init_unchained(address poolCreator_) internal initializer {
        ...
        )
    {
        ...
        tokens[0] = poolCreator.getMCBToken();
    ...
    function getAllRewards() public {
        ...
        _getReward(_mainDistribution, poolCreator.getMCBToken(), account);
        uint256 length = _subRewardTokens.length;
        for (uint256 i = 0; i < length; i++) {
            address token = _subRewardTokens[i];
    ...
    function _hasDistribution(address token) public view returns (bool) {
        return (token == poolCreator.getMCBToken()) || (_subDistributions[token].periodFinish != 0);
    }
