...
describe('Minging', () => {
    ...
    it("mining - 2 rewards", async () => {
        ...
        await miner.connect(user2).createSubDistribution(atk.address, toWei("5"), toWei("50"))
    ...
    it("mining - claim 1 by 1", async () => {
        ...
        await miner.connect(user1).setRewardRate(rtk.address, toWei("2"));
        await miner.connect(user1).notifyRewardAmount(rtk.address, toWei("40"));
        await miner.connect(user2).createSubDistribution(atk.address, toWei("1"), toWei("20"))
        // 10 round max
        for (let i = 0; i < 10; i++) {
            await stk.connect(user1).approve(miner.address, toWei("20"));
        }
        await expectRewards(miner, user1.address, [rtk.address, atk.address], [toWei("22"), toWei("10")])

        await miner.connect(user1).getReward(rtk.address)
        expect(await rtk.balanceOf(user1.address)).to.equal(toWei("24"))

        await miner.connect(user1).getReward(atk.address)
        expect(await atk.balanceOf(user1.address)).to.equal(toWei("12"))
        await expectRewards(miner, user1.address, [rtk.address, atk.address], [toWei("2"), toWei("0")])

        await miner.connect(user1).getAllRewards()
        await expectRewards(miner, user1.address, [rtk.address, atk.address], [toWei("0"), toWei("0")])

        expect(await rtk.balanceOf(user1.address)).to.equal(toWei("28"))
        expect(await atk.balanceOf(user1.address)).to.equal(toWei("13"))

        // 10 round max
        for (let i = 0; i < 10; i++) {
            await stk.connect(user1).approve(miner.address, toWei("20"));
        }
        await expectRewards(miner, user1.address, [rtk.address, atk.address], [toWei("12"), toWei("7")])
        await miner.connect(user1).getAllRewards()
        expect(await rtk.balanceOf(user1.address)).to.equal(toWei("40"))
        expect(await atk.balanceOf(user1.address)).to.equal(toWei("20"))
    ...
    it("mining - exceptions", async () => {
        const atk = await createContract("CustomERC20", ["ATK", "ATK", 6]);
        await atk.mint(miner.address, toWei("100"));
        await rtk.mint(miner.address, toWei("100"));
        await stk.mint(user1.address, toWei("100"));

        await expect(miner.setRewardRate(rtk.address, toWei("1"))).to.be.revertedWith("caller must be owner of pool creator")
        await expect(miner.setRewardRate(atk.address, toWei("1"))).to.be.revertedWith("caller must be operator")

        await miner.setOperator(user2.address);
        await expect(miner.connect(user2).setRewardRate(atk.address, toWei("1"))).to.be.revertedWith("distribution not exists")
        await expect(miner.connect(user2).notifyRewardAmount(atk.address, toWei("1"))).to.be.revertedWith("distribution not exists")

        await expect(miner.connect(user2).createSubDistribution(atk.address, toWei("0"), toWei("50"))).to.be.revertedWith("rewardRate is zero")
    })
})